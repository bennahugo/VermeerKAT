...
class input_box(npyscreen.ActionPopup):
    def __init__(self, event_loop, labeltype, labeltext, editvalue, on_ok=None, on_cancel=None, lines=5, title="User input", *args, **kwargs):
        ...
        min_width = max(len(self.__labeltext) +
                        len(self.__editvalue), len(title)) + 20
        kwargs['columns'] = kwargs['minimum_columns'] = min_width
        npyscreen.ActionPopup.__init__(
            self, *args, name=title, lines=lines, **kwargs)

    def create(self):
        self.edt_val = self.add(
            self.__labeltype, hidden=False, name=self.__labeltext, value=self.__editvalue)
...
class optioned_input_box(input_box):
    def __init__(self, event_loop, labeltext, editvalues, defaultvalue, on_ok=None, on_cancel=None, *args, **kwargs):
        ...
        min_width = max(len(self.__labeltext) + len(self.__defaultvalue),
                        len(kwargs.get('title', ""))) + 20
        kwargs['columns'] = kwargs['minimum_columns'] = min_width
        input_box.__init__(self, event_loop, npyscreen.TitleSelectOne, labeltext,
                           editvalues, on_ok, on_cancel, lines=len(self.__editvalue)+4, *args, **kwargs)

    def create(self):
        self.edt_val = self.add(npyscreen.TitleSelectOne, hidden=False, name=self.__labeltext, values=self.__editvalue,
                                value=[self.__editvalue.index(
                                    self.__defaultvalue)],
...
class message_box(action_popup_okonly):
    def __init__(self, event_loop, editvalue, lines=5, labeltext="(i)\t", title="INFO", on_ok=None, *args, **kwargs):
        ...
        min_width = max(len(self.__labeltext) +
                        len(self.__editvalue), len(title)) + 20
        kwargs['columns'] = kwargs['minimum_columns'] = min_width
        action_popup_okonly.__init__(
            self, *args, name=self.__title, lines=lines, **kwargs)
...
class warning_box(message_box):
    def __init__(self,  event_loop, editvalue, lines=5, labeltext="(!!!)\t", title="WARNING", *args, **kwargs):
        message_box.__init__(self, event_loop, editvalue, *args,
                             lines=lines, labeltext=labeltext, title=title,  **kwargs)
